...
if ! command -v ruby >/dev/null 2>&1; then
  echo "adopt-reject.sh: ruby not found on PATH; install ruby to use this repo's scripts" >&2
  exit 127
fi
# shellcheck disable=SC2034 # consumed by the sourced shared helper
ADOPT_TOOL=adopt-reject.sh
# shellcheck disable=SC1091
. "$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)/lib-adopt.sh"
usage() { echo 'Usage: adopt-reject.sh --vault <root> --topic <key> --authorization-ref <canonical reference> --reason <text> [--now <ISO8601Z>]' >&2; }
...
record="$vault/45_ai-systems/self-growth/proposals/$topic.md"; run() { adopt_run_worker reject "$record" "$now" 0 "$auth_ref" "$reason"; }; adopt_with_lock "$vault" run
