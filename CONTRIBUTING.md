...
- **ruby** — standard library only; runs on macOS system ruby 2.6+ (stock, no gems); CI runs distro rubies on both legs; additionally verified on plain `ubuntu:24.04` (ruby 3.2). Every entry script checks for it first and exits 127 when it is unavailable.
...
- **a UTF-8 locale** — the entry scripts probe `en_US.UTF-8` first and `C.UTF-8` second; Linux hosts need at least one of them available.
